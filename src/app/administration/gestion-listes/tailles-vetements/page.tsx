...
import { APPAREL_SIZES as INITIAL_APPAREL_SIZES, GLOBAL_STOCK_STORAGE_KEY } from '@/config/stock-constants';
...
const getStoredApparelSizes = (): string[] => {
  if (typeof window === 'undefined') return [...INITIAL_APPAREL_SIZES];
  const stored = localStorage.getItem(APPAREL_SIZES_STORAGE_KEY);
  if (stored) {
    try {
      ...
      console.error("Failed to parse apparel sizes from localStorage", e);
    }
  }
  const initialSizes = [...INITIAL_APPAREL_SIZES];
  localStorage.setItem(APPAREL_SIZES_STORAGE_KEY, JSON.stringify(initialSizes));
  return initialSizes;
};

const saveStoredApparelSizes = (sizes: string[]) => {
  if (typeof window === 'undefined') return;
  localStorage.setItem(APPAREL_SIZES_STORAGE_KEY, JSON.stringify(sizes));
  window.dispatchEvent(new StorageEvent('storage', { key: APPAREL_SIZES_STORAGE_KEY }));
};
...
export default function TaillesVetementsPage() {
  const [apparelSizes, setApparelSizes] = useState<string[]>([]);
  ...
  useEffect(() => {
    setApparelSizes(getStoredApparelSizes());
    setIsLoading(false);

    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === APPAREL_SIZES_STORAGE_KEY) {
        setApparelSizes(getStoredApparelSizes());
      }
    };
  ...
  const handleAddSize: SubmitHandler<AddSizeFormData> = (data) => {
    ...
    if (apparelSizes.some(s => s.toLowerCase() === newName.toLowerCase())) {
      toast({
        title: "Erreur",
    ...
    const updatedSizes = [...apparelSizes, newName].sort((a, b) => {
        return a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' });
    });
    setApparelSizes(updatedSizes);
    saveStoredApparelSizes(updatedSizes);
    addForm.reset();
    toast({
      title: "Succès",
  ...
  const requestDeleteSize = (sizeName: string) => {
    ...
    const isSizeUsedInStock = globalStockItems.some(
      (item) => item.itemCategory === 'apparel' && 
                 item.sizeBreakdown && 
    ...
    if (isSizeUsedInStock) {
      toast({
        ...
        description: `La taille "${sizeName}" est actuellement utilisée par un ou plusieurs articles de stock et ne peut pas être supprimée.`,
        variant: "destructive",
        duration: 5000,
      });
      return;
  ...
  const confirmDeleteSize = () => {
    if (itemToDelete) {
      const updatedSizes = apparelSizes.filter(s => s !== itemToDelete);
      setApparelSizes(updatedSizes);
      saveStoredApparelSizes(updatedSizes);
      toast({
        title: "Succès",
  ...
  const handleSaveEditedSize: SubmitHandler<EditSizeFormData> = (data) => {
    ...
    if (newName.toLowerCase() !== oldName.toLowerCase() && apparelSizes.some(s => s.toLowerCase() === newName.toLowerCase())) {
      toast({
        title: "Erreur",
    ...
    // Update in apparel sizes list
    const updatedSizes = apparelSizes.map(s => (s === oldName ? newName : s)).sort((a, b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
    setApparelSizes(updatedSizes);
    saveStoredApparelSizes(updatedSizes);
    ...
    const updatedStockItems = globalStockItems.map(item => {
      if (item.itemCategory === 'apparel' && item.sizeBreakdown && item.sizeBreakdown.hasOwnProperty(oldName)) {
        ...
        // Also update availableSizes if this item specifically used the old list
        let newAvailableSizes = item.availableSizes;
        if (item.availableSizes && item.availableSizes.includes(oldName)) {
            newAvailableSizes = item.availableSizes.map(s => s === oldName ? newName : s).sort((a,b) => a.localeCompare(b, undefined, { numeric: true, sensitivity: 'base' }));
        }
  ...
  return (
    <div className="container mx-auto p-4">
      <div className="mb-6 flex items-center">
        ...
        <h1 className="text-2xl font-bold">Gestion des Tailles de Vêtements</h1>
      </div>
      
      <div className="mb-6 p-4 border rounded-md bg-muted text-muted-foreground">
        <p className="text-sm font-semibold text-amber-700">Attention :</p>
        <p className="text-xs mt-1">
          La suppression ou modification d'une taille utilisée dans les définitions de stock peut causer des incohérences.
          La vérification "si utilisé" actuelle ne couvre que les articles de stock.
        </p>
      </div>
      ...
      <form onSubmit={addForm.handleSubmit(handleAddSize)} className="mb-6 flex items-start gap-2">
        <div className="flex-grow">
          <Input
            ...
            placeholder="Nom de la nouvelle taille (ex: 6 ans, S, M...)"
      ...
      {apparelSizes.length > 0 ? (
        ...
        <p className="text-center text-muted-foreground">Aucune taille de vêtement à afficher.</p>
      )}
